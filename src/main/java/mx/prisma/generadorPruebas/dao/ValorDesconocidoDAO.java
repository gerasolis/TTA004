...
public class ValorDesconocidoDAO extends GenericDAO{
	...
	public void registraRuta(String nuevaRuta, Entrada entrada){
		try {
			session.beginTransaction();
			Query query = session.createSQLQuery("INSERT INTO ValorDesconocido (id, ruta) "
					+ "VALUES ("+ entrada.getId() + ", '"+nuevaRuta + "')");
			query.executeUpdate();
			session.getTransaction().commit();
		} catch (HibernateException he) {
			he.printStackTrace();

...
(function (root, factory) {
  console.log("hola desde actor.js");
...
App = (function() {
  ...
  App.prototype.setupRootElement = function(iframe, asRoot) {
    ...
    if (iframe) {
      ...
      this.document = this.$actorInput[0].ownerDocument;
  ...
  App.prototype.reg = function(flag, setting) {
    var base, controller;
    controller = (base = this.controllers)[flag] || (base[flag] = this.$actorInput.is('[contentEditable]') ? new EditableController(this, flag) : new TextareaController(this, flag));
    if (setting.alias) {
  ...
  App.prototype.listen = function() {
    return this.$actorInput.on('compositionstart', (function(_this) {
      return function(e) {
        var ref;
  ...
  App.prototype.shutdown = function() {
    ...
    this.$actorInput.off('.atwhoInner');
...
Controller = (function() {
  ...
  function Controller(app1, at1) {
    this.app = app1;
    this.at = at1;
    this.$actorInput = this.app.$actorInput;
    this.id = this.$actorInput[0].id || this.uid();
    this.setting = null;
    this.query = null;
    this.pos = 0;
    this.range = null;
  ...
  Controller.prototype.trigger = function(name, data) {
    ...
    return this.$actorInput.trigger(eventName, data);
  };
...
TextareaController = (function(superClass) {
  ...
  TextareaController.prototype.catchQuery = function() {
    ...
    content = this.$actorInput.val();
    caretPos = this.$actorInput.caret('pos', {
  ...
  TextareaController.prototype.rect = function() {
    ...
    if (!(c = this.$actorInput.caret('offset', this.pos - 1, {
      iframe: this.app.iframe
    }))) {
      return;
    }
  ...
  TextareaController.prototype.insert = function(content, $li) {
    var $actorInput, source, startStr, suffix, text;
    $actorInput = this.$actorInput;
    source = $actorInput.val();
    startStr = source.slice(0, Math.max(this.query.headPos - this.at.length, 0));
    suffix = (suffix = this.getOpt('suffix')) === "" ? suffix : suffix || " ";
    content += suffix;
    text = "" + startStr + content + (source.slice(this.query['endPos'] || 0));
    $actorInput.val(text);
    $actorInput.caret('pos', startStr.length + content.length, {
      iframe: this.app.iframe
    });
    if (!$actorInput.is(':focus')) {
      $actorInput.focus();
    }
    return $actorInput.change();
  };
...
EditableController = (function(superClass) {
  ...
  EditableController.prototype.catchQuery = function(e) {
    ...
    if (/firefox/i.test(navigator.userAgent)) {
      if ($(range.startContainer).is(this.$actorInput)) {
        this._clearRange();
        return;
      }
  ...
  EditableController.prototype.rect = function() {
    ...
    if (this.app.iframe && !this.app.iframeAsRoot) {
      ...
      rect.left += iframeOffset.left - this.$actorInput.scrollLeft();
      rect.top += iframeOffset.top - this.$actorInput.scrollTop();
    }
  ...
  EditableController.prototype.insert = function(content, $li) {
    ...
    if (!this.$actorInput.is(':focus')) {
      this.$actorInput.focus();
    }
    return this.$actorInput.change();
  };
...
Model = (function() {
  function Model(context) {
    ...
    this.storage = this.context.$actorInput;
  }
...
Api = {
  ...
  destroy: function() {
    this.shutdown();
    return this.$actorInput.data('atwho', null);
  }
};
